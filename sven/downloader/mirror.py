...
class MirrorManager:
    ...
    def save_ranked_mirrorlist(self, mirrors: list[dict]) -> Path:
        """
        Persist a ranked mirror list for future runs and manual inspection.
        """
        path = Path(MIRRORLIST_FILE)
        path.parent.mkdir(parents=True, exist_ok=True)
        with open(path, "w") as f:
            f.write("# Generated by 'sven mirror --rank'\n")
            f.write("# Ranked fastest first\n")
